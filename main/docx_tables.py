...
# =======================
# 2 USTUNLIK (RANK | FIO) RO'YXAT
# =======================
def set_cell_text_big(cell, text, bold=False, align="left", font_size=12):
    cell.text = ""
    p = cell.paragraphs[0]
    run = p.add_run("" if text is None else str(text))
    run.font.name = "Times New Roman"
...
def create_receiver_list_table(doc, pairs, left_w=7.5, right_w=9.5, font_size=12):
    """
    pairs: [(rank_text, full_name), ...]
    Word/LibreOffice mos bo‘lishi uchun TABLE qilib chiqazamiz.
    Borders yo'q. 2 ustun.
    """
    if not pairs:
        pairs = [("", "")]
    ...
    for i, (rank_text, fio) in enumerate(pairs):
        c1 = table.rows[i].cells[0]
        c2 = table.rows[i].cells[1]

        set_cell_text_big(c1, rank_text, bold=True, align="left", font_size=font_size)
        set_cell_text_big(c2, fio, bold=True, align="left", font_size=font_size)

        # set_col_width sizda oldin bor bo‘lishi kerak
        # agar yo‘q bo‘lsa, pastdagi set_col_width ni qo‘shib qo‘ying.
        set_col_width(c1, left_w)
        set_col_width(c2, right_w)

    return table


# =======================
# AGAR set_col_width SIZDA BO'LMASA - SHUNI HAM QO'SHING
# =======================
def set_col_width(cell, cm):
    twips = int(cm * 567)
    tc = cell._tc
